...
class FirestoreService {
  ...
  /// Add comment
  Future<void> addStoreComment(String storeId, CommentModel comment) async {
    await _db
        .collection('stores')
        .doc(storeId)
        .collection('comments')
  ...
  /// Delete comment
  Future<void> deleteStoreComment(String storeId, String commentId) async {
    await _db
        .collection('stores')
        .doc(storeId)
        .collection('comments')
  ...
  Future<void> toggleStoreCommentLike({
    ...
    await _db.runTransaction((tx) async {
      ...
      tx.update(ref, {'likes': likes, 'dislikes': dislikes});
    });
  }
  ...
  Future<void> rateStore(String storeId, double rating) async {
    ...
    await _db.runTransaction((tx) async {
      ...
      tx.update(ref, {'rating': newRating, 'ratingCount': newCount});
    });
  }
  ...
  Future<event.EventModel?> getEventById(String id) async {
    ...
    return event.EventModel.fromMap(doc.data() as Map<String, dynamic>, doc.id);
  }
  ...
  Future<void> likeEvent(String eventId, String userId) async {
    ...
    await _db.runTransaction((tx) async {
      ...
      final data = snap.data()!;
      final List<String> likes = List<String>.from(data['likesList'] ?? []);
      ...
      tx.update(ref, {'likesList': likes, 'likesCount': count < 0 ? 0 : count});
    });
  }
  ...
  Future<void> toggleCommentLike({
    ...
    await _db.runTransaction((tx) async {
      ...
      final List comments = List<Map<String, dynamic>>.from(
        snap.data()!['comments'] ?? [],
      );
...
Stream<QuerySnapshot> userEventsStream(String userId) {
  ...
  Future<void> warnUser(String userId) async {
    ...
    await _db.runTransaction((tx) async {
      ...
      tx.update(ref, {'warningCount': currentWarnings + 1});
    });
  }
  ...
  Future<void> tempBanUser(String userId, int days) async {
    await _db.collection('users').doc(userId).update({
      'isBanned': true,
      'banUntil': Timestamp.fromDate(DateTime.now().add(Duration(days: days))),
    });
  }
 ...
 /// ================================
  ...
  /// Search events by title or description (status == 'approved')
  Future<List<event.EventModel>> searchEvents(String query) async {
    ...
    try {
      final snap = await _db
          .collection('events')
          .where('status', isEqualTo: 'approved') // ✅ matches your schema
          .limit(50)
          .get();
  ...
  /// Search stores by name or description (approved == true)
  Future<List<store.StoreModel>> searchStores(String query) async {
    final q = query.trim().toLowerCase();
    if (q.isEmpty) return [];

    // 🔹 Try prefix search if name_lowercase exists
    try {
      final snap = await _db
          .collection('stores')
          .where('approved', isEqualTo: true)
          .where('name_lowercase', isGreaterThanOrEqualTo: q)
          .where('name_lowercase', isLessThanOrEqualTo: '$q\uf8ff')
          .limit(20)
          .get();
